import type { AiTrainingPlanProvider, TrainingPlanAiContext } from "@/lib/ai/training-types";
...
export class MockTrainingPlanProvider implements AiTrainingPlanProvider {
  async generateExplanation(context: TrainingPlanAiContext) {
    ...
    return {
      explanation: [
        `这套计划以每周 ${request.trainingDaysPerWeek} 练为前提，采用 ${request.splitType} 结构，是为了在你当前 ${request.sessionDurationMinutes} 分钟单次时长内，把主要肌群的有效训练量平均铺开。`,
        ...
        `周安排依照 ${weeklySchedule.join("、")} 展开，连续训练日之间会切开主要疲劳部位，降低下背、肩带和肘腕的重复负担。`,
